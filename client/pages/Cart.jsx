...
const pluralGoods = (n) => (n === 1 ? "товар" : n > 1 && n < 5 ? "товара" : "товаров");

// Откуда брать userId: сначала проп/глобалка, затем localStorage, иначе 0
const resolveUserId = () => {
  if (typeof window !== "undefined") {
    if (window.__USER_ID__) return String(window.__USER_ID__);
...
const mapCartResponse = (data) => {
  ...
  return items.map((row) => ({
    id: row?.id ?? `${row?.productId ?? "p"}-${row?.variantId ?? "v"}`,
    productId: row?.productId ?? null,
    variantId: row?.variantId ?? null,
    name: row?.displayName || "Товар",
    // показываем актуальную цену, если есть; иначе — цену на момент добавления
    price: Number(row?.currentPrice ?? row?.priceAtAdded ?? 0),
    quantity: Math.max(1, Number(row?.quantity ?? 1)),
    // у бэка нет изображения/веса — подставим плейсхолдеры
...
export default function Cart() {
  ...
  const [items, setItems] = useState([]);              // ← без моков
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const [selected, setSelected] = useState(new Set()); // выставим после загрузки
  ...
  const fetchCart = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/${encodeURIComponent(userId)}/cart`, { method: "GET" });
  ...
  // ---- локальные действия (оптимистично, без вызовов PUT/DELETE, т.к. эндпоинты не заданы) ----
  const toggleAll = () => {
      ...
      <PageFade>
        <div className="container mx-auto px-4 sm:px-6 lg:px-[80px] py-8 lg:py-10">
          ...
          {!loading && !error && isEmpty && (
            ...
            >
              <img src="/empty.svg" alt="Корзина пуста" className="h-auto mb-4 w-60 sm:w-72 sm:mb-6" />
          ...
          {!loading && !error && !isEmpty && (
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6 lg:gap-8">
              ...
              <div>
                      ...
                      {items.map((i) => (
                        ...
                        >
                          <div className="grid grid-cols-[50px_110px_1fr_200px_150px_60px] items-center">
                            ...
                            <div className="pl-4">
                              <img src={i.image} alt={i.name} className="w-[80px] h-[110px] object-contain" />
                            </div>
                            ...
                            <div className="flex justify-center">
                              <div className="flex items-center justify-between w-[120px] h-[38px] border border-[#1E1E1E] rounded-full text-[16px]">
                                <button onClick={() => updateQuantity(i.id, i.quantity + 1)} className="w-10 text-lg leading-none" aria-label="Увеличить">+</button>
                                <span>{i.quantity}</span>
                                <button onClick={() => updateQuantity(i.id, i.quantity - 1)} className="w-10 text-lg leading-none" aria-label="Уменьшить">–</button>
                              </div>
                            </div>
                            ...
                            <div className="flex justify-center">
                              <button onClick={() => removeItem(i.id)} className="hover:opacity-70" aria-label="Удалить">
                          ...
                          <div>
                            <div className="flex gap-3">
                              <div className="flex-shrink-0 w-16 h-24">
                                <img src={i.image} alt={i.name} className="object-contain w-full h-full" />
                              </div>
                              <div className="flex-1">
                                <p className="text-[15px] text-[#1E1E1E] leading-tight">{i.name}</p>
                                {i.weight ? <p className="text-sm text-[#7A7A7A] mt-1">Вес: {i.weight}</p> : null}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-3">
                              ...
                              <div className="flex items-center gap-3">
                                <div className="text-[#6F2A2B] text-[16px]">{fmtMoney(i.price * i.quantity)}</div>
                                <button onClick={() => removeItem(i.id)} className="p-2 rounded hover:bg-gray-100" aria-label="Удалить">
                                  <Trash2 className="w-5 h-5 text-[#1E1E1E]" />
                                </button>
                              </div>
                            </div>
                ...
                <div className="mt-6">
                  <Link to="/catalog" className="text-[#1E1E1E] text-[15px] hover:text-[#6F2A2B]">← В каталог</Link>
                </div>
              </div>
              ...
              >
                <div className="border border-[#E2E2E2] rounded-[12px] p-5 sm:p-6 shadow-sm lg:sticky lg:top-4">
                  ...
                  <div className="mt-4 sm:mt-5">
                    <div className="text-[#6F2A2B] text-[15px] sm:text-[16px] mb-2">Итоговая стоимость</div>
                  ...
                  <div className="mt-5 sm:mt-6">
                    ...
                    <label className="flex items-center gap-2 text-[15px] mb-2">
                      <input type="radio" name="pay" value="cash" checked={payMethod === "cash"} onChange={() => setPayMethod("cash")} className="accent-[#6F2A2B]" />
                      Наличные
                    </label>
                    <label className="flex items-center gap-2 text-[15px]">
                      <input type="radio" name="pay" value="umoney" checked={payMethod === "umoney"} onChange={() => setPayMethod("umoney")} className="accent-[#6F2A2B]" />
                      Юmoney
                    </label>
                  </div>
                  ...
                  <div className="mt-5 sm:mt-6">
                    ...
                    <input value={receiver} onChange={(e) => setReceiver(e.target.value)} placeholder="ФИО получателя" className="w-full h-[40px] border border-[#E2E2E2] rounded px-3 text-[14px] mb-3 placeholder:text-[#B0B0B0]" />
                    <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Номер телефона" className="w-full h-[40px] border border-[#E2E2E2] rounded px-3 text-[14px] mb-3 placeholder:text-[#B0B0B0]" />
                    <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Адрес доставки" className="w-full h-[40px] border border-[#E2E2E2] rounded px-3 text-[14px] mb-4 placeholder:text-[#B0B0B0]" />
                    <button onClick={onPay} className="w-full h-[48px] sm:h-[50px] rounded bg-[#6F2A2B] text-white text-[15px] sm:text-[16px] hover:bg-[#5a2223]">
                      Оплатить
                    </button>
                  </div>
                </div>
